...
However, the server, defining endpoints decide how to handle the requests, retrieving or storing data operating through data layer by executing the respective query commands or managing file system. As example, the hypermedia are stored in file system of the server, since the storage in the database as encoded text could results in bad performance and more space utilization; in order to implement this, the server must decode the base64 and serialize data as shown in Listing~\ref{...}.


The implementation of the business logic is both implemented in data layer but hte server has the final filters and logic to apply. As shown in chapter~\ref{haversine_formula} to compute the distance among current location of user requester and the posts, an enriched algorithm that wrapping the same function leverage the logic, is the computation of local collection shown in Listing~\ref{...}




The local collection are computed simply grouping the collection by the post that user requester is allowed to see, then kept just the first 3/4 most used and merged with some random one.
This features permit user to have the most used collection as it moves in the physical environment, as happens for posts.
